...
public final class CFMetaData {
	...
	public final static String DEFAULT_DELTA_FUNCTION_NAME = "NoFunction";
	public final static ByteBuffer DEFAULT_KEY_NAME = ByteBufferUtil
			.bytes("KEY");

	public static final CFMetaData StatusCf = newSystemMetadata(
			SystemTable.STATUS_CF, 0, "persistent metadata for the local node",
			BytesType.instance, null);
	public static final CFMetaData HintsCf = newSystemMetadata(
			HintedHandOffManager.HINTS_CF, 1, "hinted handoff data",
			BytesType.instance, BytesType.instance);
	public static final CFMetaData MigrationsCf = newSystemMetadata(
			Migration.MIGRATIONS_CF, 2, "individual schema mutations",
			TimeUUIDType.instance, null);
	public static final CFMetaData SchemaCf = newSystemMetadata(
			Migration.SCHEMA_CF, 3, "current state of the schema",
			UTF8Type.instance, null);
	public static final CFMetaData IndexCf = newSystemMetadata(
			SystemTable.INDEX_CF, 5, "indexes that have been completed",
			UTF8Type.instance, null);
	public static final CFMetaData NodeIdCf = newSystemMetadata(
			SystemTable.NODE_ID_CF, 6, "nodeId and their metadata",
			TimeUUIDType.instance, null);
	public static final CFMetaData VersionCf = newSystemMetadata(
			SystemTable.VERSION_CF, 7, "server version information",
			UTF8Type.instance, null);
	public static final CFMetaData FunctionCf = newSystemMetadata(
			SystemTable.FUNCTION_CF, 8, "delta function information",
			UTF8Type.instance, null);
	static {
		try {
			VersionCf.keyAlias(ByteBufferUtil.bytes("component")).keyValidator(
					UTF8Type.instance).columnMetadata(
					Collections.singletonMap(ByteBufferUtil.bytes("version"),
							new ColumnDefinition(ByteBufferUtil
									.bytes("version"), UTF8Type.instance, null,
									null, null)));
	...
	private double readRepairChance; // default 1.0 (always), chance [0.0,1.0]
	// of read repair
	private boolean replicateOnWrite; // default false
	private int gcGraceSeconds; // default 864000 (ten days)
	private AbstractType defaultValidator; // default BytesType (no-op), use
	// comparator types
	private AbstractType keyValidator; // default BytesType (no-op), use
	// comparator types
	...
	private String deltaFunctionName;
	...
	public CFMetaData deltaFunctionStrategy(DeltaData prop) {
		deltadata = prop;
		return this;
	}

	public CFMetaData deltaFunctionName(String prop) {
		deltaFunctionName = prop;
		return this;
	}
	...
	private void init() {
		...
		deltaFunctionName = DEFAULT_DELTA_FUNCTION_NAME;
		...
		try {
			...
			deltadata = createDeltaFunctionStrategy(DEFAULT_DELTA_FUNCTION_NAME);
		} catch (ConfigurationException e) {
	...
	private static CFMetaData newSystemMetadata(String cfName, int cfId,
		...
		return newCFMD.comment(comment).keyCacheSize(0.01).readRepairChance(0)
				.gcGraceSeconds(0).mergeShardsChance(0.0).rowCacheSavePeriod(0)
	...
	public static CFMetaData newIndexMetadata(CFMetaData parent,
		...
		return new CFMetaData(parent.ksName,
				...
				columnComparator, null).keyValidator(info.getValidator())
				.keyCacheSize(0.0).readRepairChance(0.0).gcGraceSeconds(
						parent.gcGraceSeconds).minCompactionThreshold(
						parent.minCompactionThreshold).maxCompactionThreshold(
	...
	private static CFMetaData copyOpts(CFMetaData newCFMD, CFMetaData oldCFMD) {
		return newCFMD.comment(oldCFMD.comment).rowCacheSize(
				oldCFMD.rowCacheSize).keyCacheSize(oldCFMD.keyCacheSize)
				.readRepairChance(oldCFMD.readRepairChance).replicateOnWrite(
						oldCFMD.replicateOnWrite).gcGraceSeconds(
						oldCFMD.gcGraceSeconds).defaultValidator(
						oldCFMD.defaultValidator).minCompactionThreshold(
						oldCFMD.minCompactionThreshold).maxCompactionThreshold(
						oldCFMD.maxCompactionThreshold).rowCacheSavePeriod(
						oldCFMD.rowCacheSavePeriodInSeconds)
				.keyCacheSavePeriod(oldCFMD.keyCacheSavePeriodInSeconds)
				.rowCacheKeysToSave(oldCFMD.rowCacheKeysToSave).columnMetadata(
						oldCFMD.column_metadata).compactionStrategyClass(
				...
				.deltaFunctionStrategy(oldCFMD.deltadata).deltaFunctionName(
						oldCFMD.deltaFunctionName);
	}
	...
	public org.apache.cassandra.db.migration.avro.CfDef toAvro() {
		...
		if (subcolumnComparator != null) {
			assert cfType == ColumnFamilyType.Super : String.format(
					"%s CF %s should not have subcomparator %s defined",
		...
		cf.default_validation_class = defaultValidator == null ? null
				: new Utf8(defaultValidator.toString());
		...
		cf.delta_function_name = new Utf8(deltaFunctionName);
		if (compactionStrategyOptions != null) {
			cf.compaction_strategy_options = new HashMap<CharSequence, CharSequence>();
			for (Map.Entry<String, String> e : compactionStrategyOptions
					.entrySet())
				cf.compaction_strategy_options.put(new Utf8(e.getKey()),
						new Utf8(e.getValue()));
		}
	...
	public static CFMetaData fromAvro(
			org.apache.cassandra.db.migration.avro.CfDef cf) {
		...
		try {
			...
			throw new RuntimeException(
					"Could not inflate CFMetaData for " + cf, ex);
		}
		Map<ByteBuffer, ColumnDefinition> column_metadata = new TreeMap<ByteBuffer, ColumnDefinition>(
				BytesType.instance);
		...
		CFMetaData newCFMD = new CFMetaData(cf.keyspace.toString(), cf.name
				.toString(),
				ColumnFamilyType.create(cf.column_type.toString()), comparator,
				subcolumnComparator, cf.id);
		...
		if (cf.row_cache_provider != null) {
			try {
				newCFMD.rowCacheProvider(FBUtilities
						.newCacheProvider(cf.row_cache_provider.toString()));
			} catch (ConfigurationException e) {
				// default was already set upon newCFMD init
				logger
						.warn(
								"Unable to instantiate cache provider {}; using default {} instead",
		...
		if (cf.compaction_strategy != null) {
			try {
				newCFMD.compactionStrategyClass = createCompactionStrategy(cf.compaction_strategy
						.toString());
			} catch (ConfigurationException e) {
				throw new RuntimeException(e);
			}
		}
		if (cf.delta_function_name != null) {
			try {
				newCFMD.deltaFunctionName = cf.delta_function_name.toString();
				newCFMD.deltadata = createDeltaFunctionStrategy(cf.delta_function_name
						.toString());
			} catch (ConfigurationException e) {
				throw new RuntimeException(e);
			}
		...
		if (cf.compaction_strategy_options != null) {
			for (Map.Entry<CharSequence, CharSequence> e : cf.compaction_strategy_options
					.entrySet())
				newCFMD.compactionStrategyOptions.put(e.getKey().toString(), e
						.getValue().toString());
		}
		...
		return newCFMD.comment(cf.comment.toString()).rowCacheSize(
				cf.row_cache_size).keyCacheSize(cf.key_cache_size)
				.readRepairChance(cf.read_repair_chance).replicateOnWrite(
						cf.replicate_on_write).gcGraceSeconds(
						cf.gc_grace_seconds).defaultValidator(validator)
				.keyValidator(keyValidator).columnMetadata(column_metadata)
				.compressionParameters(cp);
	}
	...
	public boolean equals(Object obj) {
		...
		return new EqualsBuilder().append(ksName, rhs.ksName).append(cfName,
				rhs.cfName).append(cfType, rhs.cfType).append(comparator,
				rhs.comparator).append(subcolumnComparator,
				rhs.subcolumnComparator).append(comment, rhs.comment).append(
				rowCacheSize, rhs.rowCacheSize).append(keyCacheSize,
				rhs.keyCacheSize)
				.append(readRepairChance, rhs.readRepairChance).append(
						replicateOnWrite, rhs.replicateOnWrite).append(
						gcGraceSeconds, rhs.gcGraceSeconds).append(
						defaultValidator, rhs.defaultValidator).append(
						keyValidator, rhs.keyValidator).append(
						minCompactionThreshold, rhs.minCompactionThreshold)
				.append(maxCompactionThreshold, rhs.maxCompactionThreshold)
				.append(cfId.intValue(), rhs.cfId.intValue()).append(
						column_metadata, rhs.column_metadata).append(
						rowCacheSavePeriodInSeconds,
						rhs.rowCacheSavePeriodInSeconds).append(
						keyCacheSavePeriodInSeconds,
						rhs.keyCacheSavePeriodInSeconds).append(
						rowCacheKeysToSave, rhs.rowCacheKeysToSave).append(
						mergeShardsChance, rhs.mergeShardsChance).append(
						keyAlias, rhs.keyAlias).append(compactionStrategyClass,
						rhs.compactionStrategyClass).append(deltadata,
						rhs.deltadata).append(deltaFunctionName,
						rhs.deltaFunctionName).append(
						compactionStrategyOptions,
						rhs.compactionStrategyOptions).append(
						compressionParameters, rhs.compressionParameters)
				.isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder(29, 1597).append(ksName).append(cfName)
				.append(cfType).append(comparator).append(subcolumnComparator)
				.append(comment).append(rowCacheSize).append(keyCacheSize)
				.append(readRepairChance).append(replicateOnWrite).append(
						gcGraceSeconds).append(defaultValidator).append(
						keyValidator).append(minCompactionThreshold).append(
						maxCompactionThreshold).append(cfId).append(
						column_metadata).append(rowCacheSavePeriodInSeconds)
				.append(keyCacheSavePeriodInSeconds).append(rowCacheKeysToSave)
				.append(mergeShardsChance).append(keyAlias).append(
						compactionStrategyClass).append(deltadata).append(
						deltaFunctionName).append(compactionStrategyOptions)
				.append(compressionParameters).toHashCode();
	}
	...
	public static void applyImplicitDefaults(
		...
		if (!cf_def.isSetMin_compaction_threshold())
			cf_def
					.setMin_compaction_threshold(CFMetaData.DEFAULT_MIN_COMPACTION_THRESHOLD);
		if (!cf_def.isSetMax_compaction_threshold())
			cf_def
					.setMax_compaction_threshold(CFMetaData.DEFAULT_MAX_COMPACTION_THRESHOLD);
		if (!cf_def.isSetRow_cache_save_period_in_seconds())
			cf_def
					.setRow_cache_save_period_in_seconds(CFMetaData.DEFAULT_ROW_CACHE_SAVE_PERIOD_IN_SECONDS);
		if (!cf_def.isSetKey_cache_save_period_in_seconds())
			cf_def
					.setKey_cache_save_period_in_seconds(CFMetaData.DEFAULT_KEY_CACHE_SAVE_PERIOD_IN_SECONDS);
		if (!cf_def.isSetRow_cache_keys_to_save())
			cf_def
					.setRow_cache_keys_to_save(CFMetaData.DEFAULT_ROW_CACHE_KEYS_TO_SAVE);
		if (!cf_def.isSetMerge_shards_chance())
			cf_def
					.setMerge_shards_chance(CFMetaData.DEFAULT_MERGE_SHARDS_CHANCE);
		if (null == cf_def.compaction_strategy)
			cf_def.compaction_strategy = DEFAULT_COMPACTION_STRATEGY_CLASS;
		if (null == cf_def.delta_function_name)
			cf_def.delta_function_name = DEFAULT_DELTA_FUNCTION_NAME;
	...
	public static CFMetaData fromThrift(org.apache.cassandra.thrift.CfDef cf_def)
		...
		CFMetaData newCFMD = new CFMetaData(cf_def.keyspace, cf_def.name,
				...
				cf_def.subcomparator_type == null ? null : TypeParser
		...
		if (cf_def.isSetRow_cache_provider()) {
			newCFMD.rowCacheProvider(FBUtilities
					.newCacheProvider(cf_def.row_cache_provider));
		}
		...
		if (cf_def.isSetDelta_function_name()) {
			newCFMD.deltaFunctionName(cf_def.delta_function_name);
			newCFMD.deltadata = createDeltaFunctionStrategy(cf_def.delta_function_name);
		}
		...
		return newCFMD.comment(cf_def.comment).rowCacheSize(
				cf_def.row_cache_size).keyCacheSize(cf_def.key_cache_size)
				.readRepairChance(cf_def.read_repair_chance).replicateOnWrite(
						cf_def.replicate_on_write).defaultValidator(
				...
				.compressionParameters(cp).validate();
	}
	...
	public void apply(org.apache.cassandra.db.migration.avro.CfDef cf_def)
		...
		} else if (subcolumnComparator != TypeParser
				.parse(cf_def.subcomparator_type))
			throw new ConfigurationException(
					"subcolumncomparators do not match.");
		...
		if (cf_def.row_cache_provider != null)
			rowCacheProvider = FBUtilities
					.newCacheProvider(cf_def.row_cache_provider.toString());
		...
		for (org.apache.cassandra.db.migration.avro.ColumnDef def : cf_def.column_metadata) {
			...
			oldDef.setIndexType(def.index_type == null ? null
					: org.apache.cassandra.thrift.IndexType
							.valueOf(def.index_type.name()), ColumnDefinition
					.getStringMap(def.index_options));
		...
		for (org.apache.cassandra.db.migration.avro.ColumnDef def : toAdd) {
			...
			ColumnDefinition cd = new ColumnDefinition(def.name, dValidClass,
					def.index_type == null ? null
							: org.apache.cassandra.thrift.IndexType
									.valueOf(def.index_type.toString()),
		...
		if (cf_def.delta_function_name != null) {
			deltaFunctionName = cf_def.delta_function_name.toString();
			deltadata = createDeltaFunctionStrategy(cf_def.delta_function_name
					.toString());
		}

		if (null != cf_def.compaction_strategy_options) {
			compactionStrategyOptions = new HashMap<String, String>();
			for (Map.Entry<CharSequence, CharSequence> e : cf_def.compaction_strategy_options
					.entrySet())
				compactionStrategyOptions.put(e.getKey().toString(), e
						.getValue().toString());
		}
	...
	public static Class<? extends AbstractCompactionStrategy> createCompactionStrategy(
		...
		try {
			return (Class<? extends AbstractCompactionStrategy>) Class
					.forName(className);
		} catch (Exception e) {
			throw new ConfigurationException(
					"Could not create Compaction Strategy of type " + className,
					e);
		}
	}

	public static DeltaData createDeltaFunctionStrategy(String className)
			throws ConfigurationException {
		String classPath;
		if (DEFAULT_DELTA_FUNCTION_NAME.equalsIgnoreCase(className)) {
			classPath = "org.apache.cassandra.delta.NoDelta";
		} else {
			FTMetaData ftm = DeltaFunction.instance.getFTMetaData(className);
			if (ftm != null) {
				classPath = ftm.strategyClass;
			} else {
				throw new ConfigurationException(
						"Could not create Delta Function of name <" + className
								+ ">.");
			}
		}
		try {
			return (DeltaData) Class.forName(classPath).newInstance();
		} catch (Exception e) {
			throw new ConfigurationException(
					"Could not create Delta Function Strategy of path "
							+ classPath, e);
		}
	}
	...
	public AbstractCompactionStrategy createCompactionStrategyInstance(
		...
		try {
			Constructor constructor = compactionStrategyClass
					.getConstructor(new Class[] { ColumnFamilyStore.class,
							Map.class // options
					});
			return (AbstractCompactionStrategy) constructor
					.newInstance(new Object[] { cfs, compactionStrategyOptions });
	...
	public org.apache.cassandra.thrift.CfDef toThrift() {
		...
		def.setDelta_function_name(deltaFunctionName);
	...
	public static void validateMinMaxCompactionThresholds(
		...
		if (cf_def.min_compaction_threshold != null
			...
			if ((cf_def.min_compaction_threshold > cf_def.max_compaction_threshold)
				...
				throw new ConfigurationException(
						"min_compaction_threshold cannot be greater than max_compaction_threshold");
			}
		} else if (cf_def.min_compaction_threshold != null) {
			if (cf_def.min_compaction_threshold > DEFAULT_MAX_COMPACTION_THRESHOLD) {
				throw new ConfigurationException(
						"min_compaction_threshold cannot be greather than max_compaction_threshold (default "
								+ DEFAULT_MAX_COMPACTION_THRESHOLD + ")");
			}
		} else if (cf_def.max_compaction_threshold != null) {
			if (cf_def.max_compaction_threshold < DEFAULT_MIN_COMPACTION_THRESHOLD
					&& cf_def.max_compaction_threshold != 0) {
				throw new ConfigurationException(
						"max_compaction_threshold cannot be less than min_compaction_threshold");
			}
		} else {
			// Defaults are valid.
		}
	 ...
	 */
	public static void addDefaultIndexNames(
			org.apache.cassandra.thrift.CfDef cf_def)
		...
		for (org.apache.cassandra.thrift.ColumnDef column : cf_def.column_metadata) {
			if (column.index_type != null && column.index_name == null)
				column.index_name = getDefaultIndexName(cf_def.name,
						comparator, column.name);
		}
	}
	...
	public String toString() {
		return new ToStringBuilder(this).append("cfId", cfId).append("ksName",
				ksName).append("cfName", cfName).append("cfType", cfType)
				.append("comparator", comparator).append("subcolumncomparator",
						subcolumnComparator).append("comment", comment).append(
						"rowCacheSize", rowCacheSize).append("keyCacheSize",
						keyCacheSize).append("readRepairChance",
						readRepairChance).append("replicateOnWrite",
						replicateOnWrite).append("gcGraceSeconds",
						gcGraceSeconds).append("defaultValidator",
						defaultValidator).append("keyValidator", keyValidator)
				.append("minCompactionThreshold", minCompactionThreshold)
				.append("maxCompactionThreshold", maxCompactionThreshold)
				.append("rowCacheSavePeriodInSeconds",
						rowCacheSavePeriodInSeconds).append(
						"keyCacheSavePeriodInSeconds",
						keyCacheSavePeriodInSeconds).append(
						"rowCacheKeysToSave", rowCacheKeysToSave).append(
						"rowCacheProvider", rowCacheProvider).append(
						"mergeShardsChance", mergeShardsChance).append(
						"keyAlias", keyAlias).append("column_metadata",
						column_metadata).append("compactionStrategyClass",
						compactionStrategyClass).append("deltadata", deltadata)
				.append("deltaFunctionName", deltaFunctionName).append(
						"compactionStrategyOptions", compactionStrategyOptions)
				.append("compressionOptions",
						compressionParameters.asThriftOptions()).toString();
	}
}
