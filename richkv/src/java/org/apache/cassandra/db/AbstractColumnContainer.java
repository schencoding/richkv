...
public abstract class AbstractColumnContainer implements IColumnContainer,
	...
	private static Logger logger = LoggerFactory
			.getLogger(AbstractColumnContainer.class);

	protected final AtomicReference<DeletionInfo> deletionInfo = new AtomicReference<DeletionInfo>(
			new DeletionInfo());
	...
	@Deprecated
	// TODO this is a hack to set initial value outside constructor
	...
	public void delete(AbstractColumnContainer cc2) {
		...
		while (true) {
			...
			if (current.markedForDeleteAt >= cc2Info.markedForDeleteAt
					|| deletionInfo.compareAndSet(current, cc2Info))
				break;
		}
	}
	...
	/**
	 * Drops expired row-level tombstones. Normally, these are dropped once the
	 * row no longer exists, but if new columns are inserted into the row
	 * post-deletion, they can keep the row tombstone alive indefinitely, with
	 * non-intuitive results. See
	 * https://issues.apache.org/jira/browse/CASSANDRA-2317
	 */
	public void maybeResetDeletionTimes(int gcBefore) {
		while (true) {
			DeletionInfo current = deletionInfo.get();
			// Stop if either we don't need to change the deletion info (it's
			// still MIN_VALUE or not expired yet) or we've succesfully changed
			// it
	...
	/**
	 * We need to go through each column in the column container and resolve it
	 * before adding
	 */
	public void addAll(AbstractColumnContainer cc, Allocator allocator) {
		for (IColumn column : cc.columns) {
			column.setDeltaData(((ColumnFamily) cc).metadata().deltadata);
		}
