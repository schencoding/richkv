...
public class ColumnSerializer implements IColumnSerializer {
	private static final Logger logger = LoggerFactory
			.getLogger(ColumnSerializer.class);

	public final static int DELETION_MASK = 0x01;
	public final static int EXPIRATION_MASK = 0x02;
	public final static int COUNTER_MASK = 0x04;
	public final static int COUNTER_UPDATE_MASK = 0x08;
	public final static int UPDATE_MASK = 0x10;
	...
	public void serialize(IColumn column, DataOutput dos) {
		...
		try {
			...
			} else if (column instanceof UpdateColumn) {
				dos.writeInt(((UpdateColumn) column).versionCount());
				dos.writeBoolean(((UpdateColumn) column).isDelta());
				Collection<IColumn> columns = ((UpdateColumn) column)
						.getSortedColumns();
				int count = columns.size();
				dos.writeInt(count);
				for (IColumn column1 : columns) {
					serialize(column1, dos);
				}
			}
			dos.writeLong(column.timestamp());
			dos.writeLong(column.version());
			if (column.value() != null) {
				ByteBufferUtil.writeWithLength(column.value(), dos);
			} else {
				ByteBufferUtil.writeWithLength(AppCase.write(column
						.getObjValue(),column.getDeltaData()), dos);
			}
		} catch (IOException e) {
	 ...
	 * deserialize comes from a remote host. If it does, then we must clear the
	 * delta.
	 */
	public Column deserialize(DataInput dis, IColumnSerializer.Flag flag)
			throws IOException {
	...
	public Column deserialize(DataInput dis, IColumnSerializer.Flag flag,
		...
		if (name.remaining() <= 0) {
			...
			if (dis instanceof FileDataInput) {
				...
				details = String.format(" (%s, %d bytes remaining)", fdis
						.getPath(), fdis.bytesRemaining());
			}
			throw new CorruptColumnException(String.format(format, name
					.remaining(), details));
		}
		...
		if ((b & COUNTER_MASK) != 0) {
			...
			return CounterColumn.create(name, value, ts, timestampOfLastDelete,
					flag);
			...
			long version = dis.readLong();
			ByteBuffer value = ByteBufferUtil.readWithLength(dis);
			return ExpiringColumn.create(name, value, ts, ttl, expiration,
					expireBefore, flag);
		} else if ((b & UPDATE_MASK) != 0) {
			int versionCount = dis.readInt();
			boolean isDelta = dis.readBoolean();
			int size = dis.readInt();
			ISortedColumns columns = UpdateColumn.createSortedColumns();
			for (int i = 0; i < size; i++) {
				IColumn column = deserialize(dis);
				columns.addColumn(column, HeapAllocator.instance);
			}
			long ts = dis.readLong();
			long version = dis.readLong();
			ByteBuffer value = ByteBufferUtil.readWithLength(dis);
			return UpdateColumn.create(name, value, ts, version, versionCount,
					isDelta, columns, flag);
		} else {
			long ts = dis.readLong();
			long version = dis.readLong();
			ByteBuffer value = ByteBufferUtil.readWithLength(dis);
			return (b & COUNTER_UPDATE_MASK) != 0 ? new CounterUpdateColumn(
					name, value, ts) : ((b & DELETION_MASK) == 0 ? new Column(
					name, value, ts, version) : new DeletedColumn(name, value,
					ts));
		}
	}
