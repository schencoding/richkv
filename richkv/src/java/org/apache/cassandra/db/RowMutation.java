...
public class RowMutation implements IMutation, MessageProducer {
	...
	public static RowMutation hintFor(RowMutation mutation, ByteBuffer token)
		...
		rm.add(path, ByteBuffer.wrap(mutation
				...
				.currentTimeMillis(), ttl, 0);

		// serialization version
		path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId,
				ByteBufferUtil.bytes("version"));
		rm.add(path, ByteBufferUtil.bytes(MessagingService.version_), System
				.currentTimeMillis(), ttl, 0);

		// table
		path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId,
				ByteBufferUtil.bytes("table"));
		rm.add(path, ByteBufferUtil.bytes(mutation.getTable()), System
				.currentTimeMillis(), ttl, 0);

		// key
		path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId,
				ByteBufferUtil.bytes("key"));
		rm.add(path, mutation.key(), System.currentTimeMillis(), ttl, 0);
	...
	public void add(QueryPath path, ByteBuffer value, long timestamp,
		...
		columnFamily.addColumn(path, value, timestamp, timeToLive, version);
	}

	public void update(QueryPath path, ByteBuffer value, long timestamp,
			long version) {
		Integer id = Schema.instance.getId(table_, path.columnFamilyName);
		ColumnFamily columnFamily = modifications_.get(id);
		if (columnFamily == null) {
		...
		columnFamily.addDelta(path, value, timestamp, version);
	...
	public void delete(QueryPath path, long timestamp) {
		...
		if (path.superColumnName == null && path.columnName == null) {
			...
			SuperColumn sc = new SuperColumn(path.superColumnName, columnFamily
					.getSubComparator());
	...
	public Message getMessage(StorageService.Verb verb, int version)
			throws IOException {
	...
	public static class RowMutationSerializer implements
		...
		public RowMutation deserialize(DataInput dis, int version,
				IColumnSerializer.Flag flag) throws IOException {
			...
			for (int i = 0; i < size; ++i) {
				...
				ColumnFamily cf = ColumnFamily.serializer().deserialize(dis,
						flag, ThreadSafeSortedColumns.factory());
