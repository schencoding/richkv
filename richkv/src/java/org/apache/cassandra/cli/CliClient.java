...
public class CliClient
{
    ...
    /*
     * the <i>add function</i> command requires a list of arguments,
     *  this enum defines which arguments are valid
     */
    private enum AddFunctionArgument {
        PLACEMENT_STRATEGY,
        DESCRIPTION
    }
    ...
    private Map<String, FtDef> functionMap = new HashMap<String, FtDef>();
        ...
        try
        {
            switch (tree.getType())
            {
                ...
                case CliParser.NODE_ADD_KEYSPACE:
                    ...
                    break;
                case CliParser.NODE_ADD_FUNCTION:
                    executeAddFunction(tree.getChild(0));
                    break;
                ...
                case CliParser.NODE_SHOW_KEYSPACES:
                    ...
                    break;
                case CliParser.NODE_SHOW_FUNCTIONS:
                    executeShowFunctions();
                    break;
     ...
     * @param statement - a token tree representing current statement
     */
    private void executeAddFunction(Tree statement)
    {

        if (!CliMain.isConnected())
            return;
        
        // first value is the function name, after that it is all key=value
        String functionName = statement.getChild(0).getText();
        FtDef ftDef = new FtDef(functionName,"");
        ftDef.setDescription("");

        try
        {
            String myFunctionVersion = thriftClient.system_add_function(updateFtDefAttributes(statement, ftDef));
            sessionState.out.println(myFunctionVersion);
            validateFunctionIsSettled(myFunctionVersion);
            functionMap.put(functionName, thriftClient.describe_function(functionName));
        }
        catch (InvalidRequestException e)
        {
            throw new RuntimeException(e);
    ...
    /**
     * Used to update function definition attributes
     * @param statement - ANTRL tree representing current statement
     * @param ftDefToUpdate - function definition to update
     * @return ftDef - updated function definition
     */
    private FtDef updateFtDefAttributes(Tree statement, FtDef ftDefToUpdate)
    {
        FtDef ftDef = new FtDef(ftDefToUpdate);

        
        for(int i = 1; i < statement.getChildCount(); i += 2)
        {
            String currentStatement = statement.getChild(i).getText().toUpperCase();
            AddFunctionArgument mArgument = AddFunctionArgument.valueOf(currentStatement);
            String mValue = statement.getChild(i + 1).getText();

            switch(mArgument)
            {
            case PLACEMENT_STRATEGY: 
                ftDef.setStrategy_class(CliUtils.unescapeSQLString(mValue));
                break;
            case DESCRIPTION:
            	ftDef.setDescription(CliUtils.unescapeSQLString(mValue));
                break;
            default:
                //must match one of the above or we'd throw an exception at the valueOf statement above.
                assert(false);
    ...
    private CfDef updateCfDefAttributes(Tree statement, CfDef cfDefToUpdate)
    {
        ...
        for (int i = 1; i < statement.getChildCount(); i += 2)
        {
            ...
            case COMPRESSION_OPTIONS:
                ...
                break;
            case DELTA_FUNCTION_NAME:
                cfDef.setDelta_function_name(CliUtils.unescapeSQLString(mValue));
                break;
            default:
                //must match one of the above or we'd throw an exception at the valueOf statement above.
                assert(false);
    ...
    // SHOW FUNCTIONS
    private void executeShowFunctions() throws TException, InvalidRequestException
    {
        if (!CliMain.isConnected())
            return;

        List<FtDef> Functions = thriftClient.describe_functions();

        Collections.sort(Functions, new FtDefNamesComparator());
        for (FtDef Function : Functions)
        {
            describeFunction(Function.name, Function);
        }
    }
    ...
    private void showColumnFamily(StringBuilder sb, CfDef cfDef)
    {
        ...
        writeAttr(sb, false, "delta_function_name", cfDef.delta_function_name);
    ...
    private void describeFunction(String functionName, FtDef metadata) throws TException
    {

        // Describe and display
        sessionState.out.println("Function: " + functionName + ":");
        try
        {
            FtDef ft_def;
            ft_def = metadata == null ? thriftClient.describe_function(functionName) : metadata;
            sessionState.out.println("  -Strategy: " + ft_def.strategy_class);
            sessionState.out.println("  -Description: " + ft_def.description);
            sessionState.out.println("-----------------------------------------");
        ...
        {
            sessionState.out.println("Function " + functionName + " could not be found.");
        }
    }
    ...
    private void describeColumnFamily(KsDef ks_def, CfDef cf_def, NodeProbe probe) throws TException
    {
        ...
        sessionState.out.printf("      Delta Function Name: %s%n", cf_def.delta_function_name);
    ...
    private static class FtDefNamesComparator implements Comparator<FtDef>
    {
        public int compare(FtDef a, FtDef b)
        {
            return a.name.compareTo(b.name);
        }
    }
    ...
    /** validates function is propagated to all nodes */
    private void validateFunctionIsSettled(String currentFunctionId)
    {
        sessionState.out.println("Waiting for delta function agreement...");
        Map<String, List<String>> versions = null;
        ...
        while (limit - System.currentTimeMillis() >= 0 && !inAgreement)
        {
            try
            {
                versions = thriftClient.describe_function_versions(); // getting function version for nodes of the ring
            }
            catch (Exception e)
            {
                sessionState.err.println((e instanceof InvalidRequestException) ? ((InvalidRequestException) e).getWhy() : e.getMessage());
            ...
            for (String version : versions.keySet())
            {
                if (!version.equals(currentFunctionId) && !version.equals(StorageProxy.UNREACHABLE))
                    continue outer;
            }
            inAgreement = true;
        }
        ...
        if (!inAgreement)
        {
            sessionState.err.printf("The function has not settled in %d seconds; further migrations are ill-advised until it does.%nVersions are %s%n",
                                    sessionState.schema_mwt / 1000, FBUtilities.toString(versions));
            System.exit(-1);
        }
        sessionState.out.println("... functions agree across the cluster");
    }
